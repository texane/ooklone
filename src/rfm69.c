...
static void rfm69_setup(void)
{
  ...
  /* fxosc = 32MHz */
#define RFM69_BITRATE_KBPS 10
  rfm69_write_bitrate((uint16_t)(32000.0 / (double)RFM69_BITRATE_KBPS));

  /* 433.92 MHz carrier frequency */
  /* fcar = fstep * frf */
  /* fstep = fxosc / (2^19) */
  /* fxosc = 32MHz */
  rfm69_write_frf((uint32_t)(433.92 / 32.0));

 /* TODO: rfm69_write_ook_peak(); */
 /* TODO: rfm69_write_ook_avg(); */
 /* TODO: rfm69_write_ook_fix(); */
}

static void rfm69_set_rx_continuous_mode(void)
{
  rfm69_write_data_modul((3 << 5) | (1 << 3));
  rfm69_write_op_mode(4 << 2);
}
